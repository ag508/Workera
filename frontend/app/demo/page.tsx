...
import { MOCK_JOBS } from "@/lib/demo-data"
import { Search, Plus, MapPin, Users, Clock, MoreHorizontal } from "lucide-react"
import { GlassCard } from "@/components/ui/glass-card"
import TiltedCard from "@/components/reactbits/TiltedCard"

export default function DemoJobsPage() {
  const [search, setSearch] = useState("")
  const filtered = MOCK_JOBS.filter(j => j.title.toLowerCase().includes(search.toLowerCase()))
    ...
    <div className="space-y-6">
       ...
       <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-gray-500">View and manage job listings.</p>
          </div>
             ...
             <TiltedCard key={job.id} className="h-full">
                <GlassCard className="h-full border border-gray-200 bg-white/50 p-6 hover:shadow-xl transition-all">
                   <div className="flex justify-between items-start mb-4">
                      <div className={`px-3 py-1 rounded-full text-xs font-medium ${job.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                         {job.status}
                      </div>
